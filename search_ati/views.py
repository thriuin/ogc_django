...
class AtiSearchView(View):
    def get(self, request):
        ...
        if len(context['organizations_selected']) > 0:
            context['orgs_expanded'] = True
        else:
            context['orgs_expanded'] = False
        if query_terms != '*':
            query_terms = "{0} or request_number_s:{1}".format(query_terms, solr_search_terms)
